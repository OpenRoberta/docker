...
LOG_CONFIG_FILE='/logback-prod.xml'  # logback configuration
...
GIT_PULL_BEFORE_BUILD=true # set it to 'true'! Set to 'false' ONLY, if you have a well-prepared, ready to use branch checked out in a repository in the
                           # 'git' directory, that is used exclusively for this server instance. Setting to 'false' is very dangerous!
...
# START_ARGS='-d key1=val1 -d key2=val2 -D robot3:key3=val3 -D robot3:key3=val3 '
                           ...
                           # -d sets/replaces server global properties from 'openroberta.properties'
                           # -D sets/replaces properties found in <robot>.properties from a robot plugin
